/*
 * @Author: mxxxOrz
 * @Date: 2024-09-04 11:26:56
 * @LastEditors: Please set LastEditors
 * @LastEditTime: 2024-09-04 20:35:54
 * @Description: 请填写简介
 */
...
// 32 bit unsigned int, 2^8 = 256 0-255
typedef vector<uint32_t> DescType;

/**
 * @description: 计算ORB描述子
 * @param {Mat} &img
 * @param {vector<cv::KeyPoint>} &keypoints
 * @param {vector<DescType>} &descriptors
 * @return {*}
 */
...
int main(int argc, char *argv[])
{
    ...
    vector<cv::KeyPoint> keypoints1; // 关键点
    cv::FAST(first_image, keypoints1, 40);
    vector<DescType> descriptor1; // 描述子
    ...
    cv::Mat img_show;
    cv::drawMatches(first_image, keypoints1, second_image, keypoints2, matches, img_show);
    ...
    for (auto &kp : keypoints)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            for (int k = 0; k < 32; k++)
            {
                ...
                // 对偏移量做旋转变换，旋转角度为theta
                cv::Point2f pp = cv::Point2f(cos_theta * p.x - sin_theta * p.y, sin_theta * p.x + cos_theta * p.y) + kp.pt;
                cv::Point2f qq = cv::Point2f(cos_theta * q.x - sin_theta * q.y, cos_theta * q.y + sin_theta * q.x) + kp.pt;
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matchces)
{
    const int d_max = 40;
